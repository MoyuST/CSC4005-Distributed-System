...
public:
    ...
    void update_for_tick(double elapse,
        ...
        #pragma omp barrier
            ...
            int startPosition = (size()/totalThreadSize)*taskNum + std::min(static_cast<int> (size())%totalThreadSize, taskNum);
            int endPosition = (size()/totalThreadSize)*(taskNum+1) + std::min(static_cast<int> (size())%totalThreadSize, taskNum+1)-1;
