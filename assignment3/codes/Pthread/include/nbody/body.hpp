...
public:
    ...
    void update_for_tick(double elapse, double gravity, double position_range, double radius, int totalThreadSize, BodyPool * pool) {
        ax.assign(size(), 0);
        ay.assign(size(), 0);
    ...
    static void * update_for_tick_inside(void *arg_ptr) {
        ...
        int startPosition = (copyPool.size()/totalThreadSize)*taskNum + std::min(static_cast<int> (copyPool.size())%totalThreadSize, taskNum);
        int endPosition = (copyPool.size()/totalThreadSize)*(taskNum+1) + std::min(static_cast<int> (copyPool.size())%totalThreadSize, taskNum+1)-1;
