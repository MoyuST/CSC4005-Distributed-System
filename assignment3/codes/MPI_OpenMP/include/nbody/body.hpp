...
public:
    ...
    void update_for_tick(double elapse,
        ...
        int processStartPosition = (size()/processSize)*rank + std::min(static_cast<int> (size())%processSize, rank);
        int processEndPosition = (size()/processSize)*(rank+1) + std::min(static_cast<int> (size())%processSize, rank+1)-1;
        int processTaskSize = processEndPosition - processStartPosition + 1;

        // printf("rank %d %d %d %d\n", rank, processStartPosition, processEndPosition, processTaskSize);
        ...
        #pragma omp barrier
            ...
            // printf("thread %d %d %d (rank %d)\n", taskNum, startPosition, endPosition, rank);
            ...
            for (size_t i = 0; i < static_cast<size_t>(startPosition); ++i) {
                for (size_t j = startPosition; j <= static_cast<size_t>(endPosition); ++j) {
                    check_and_update(copyPool.get_body(i), copyPool.get_body(j), radius, gravity);
                }
            }
