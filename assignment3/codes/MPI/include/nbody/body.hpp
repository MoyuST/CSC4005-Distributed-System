...
public:
    ...
    void update_for_tick(double elapse,
        ...
        int startPosition = (size()/processSize)*rank + std::min(static_cast<int> (size())%processSize, rank);
        int endPosition = (size()/processSize)*(rank+1) + std::min(static_cast<int> (size())%processSize, rank+1)-1;
