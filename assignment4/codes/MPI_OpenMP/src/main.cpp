    ...
    if(rank == 0){
        ...
        context.run([&](graphic::GraphicContext *context [[maybe_unused]], SDL_Window *) {
            ...
            if (!finished) {
                ...
                // printf("threads %d\tone iteration %lld (ns)\troom size %d\n", 
                // processSize*totalThreadSize, std::chrono::duration_cast<std::chrono::nanoseconds>(endIteration - beginIteration).count(), current_state.room_size);
